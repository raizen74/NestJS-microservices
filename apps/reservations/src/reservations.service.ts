...
export class ReservationsService {
    ...
    .pipe(
      map((res) => {
        const reservation = new Reservation({
          ...
          return this.reservationsRepository.create(reservation); // pipe is executed immediately after response gets back successfully, map transforms the response
        }),
      );
  }
  ...
  async findOne(id: number) {  // TypeORM works with number ids
